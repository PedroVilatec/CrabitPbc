...
if ! sudo -u pi ssh pi@10.10.0.1 "grep '$pubkey' /home/pi/.ssh/authorized_keys" > /dev/null; then
    echo "Chave ssh não existe, copiando..."
    sudo -u pi ssh-copy-id pi@10.10.0.1

fi

sudo -u pi bash -c "ssh -T pi@10.10.0.1 'cd /home/pi/Crabit/vilatec && git pull'"
sudo -u pi rsync -avu --delete --exclude="*frp*" --exclude="*libraries*" --exclude="*.pyc" --exclude="__init__.py" --exclude="__pycache__" pi@10.10.0.1:Crabit/vilatec /home/pi/Crabit/
sudo cp /home/pi/Crabit/vilatec/update_app.sh /bin/update_app
sudo chmod +x /bin/update_app
...
grep -q -e 'BOMBEAMENTO_FORCADO' /home/pi/Crabit/instance/config.py || sed -i.bak '/class Config():/a \ \ \ \ BOMBEAMENTO_FORCADO = False #Habilita o bombeamento do fluido mesmo com o nivel baixo' /home/pi/Crabit/instance/config.py
...
echo -e 'sudo killall gunicorn\nsudo gunicorn --worker-class eventlet -w 1 --reload --bind 0.0.0.0:8000 --chdir /home/pi/Crabit/vilatec app:app' | sudo tee /bin/start_app > /dev/null
sudo chmod +x /bin/start_app
...
# sudo sed -i.bak "s/b8950bd6c40e.sn.mynetname.net/10.8.0.10/g" "/bin/update_app"