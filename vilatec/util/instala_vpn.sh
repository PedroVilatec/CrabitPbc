...
# sudo scp pi@170.79.168.234:Crabit/vilatec/util/instala_vpn.sh capela/ihm/util/
...
# Copia a chave pública para o servidor intermediário 170.79.168.234
sudo -u pi bash -c 'ssh-copy-id pi@170.79.168.234'

# Solicita ao usuário para inserir o valor do campo
read -p "Digite o nome de acesso do cliente: " campo

# Comando remoto para o servidor intermediário 170.79.168.234, passando o valor de "campo"
cmd="./cria_credenciais.sh ${campo}"
sudo -u pi bash -c "ssh pi@170.79.168.234 'ssh -T pi@10.8.0.1 ${cmd}'"

# Comando remoto para o servidor intermediário 170.79.168.234, usando diretamente o valor de "campo"
sudo -u pi bash -c "ssh pi@170.79.168.234 'cd /home/pi/vpn && git pull'"


# Monta o comando scp com o campo fornecido pelo usuário
cd /etc/openvpn
comando_scp="scp -i /home/pi/.ssh/id_ed25519 pi@170.79.168.234:vpn/${campo}/*.* ."
$comando_scp
# comando_scp="scp -i /home/pi/.ssh/id_ed25519 pi@170.79.168.234:/home/pi/vpn/${campo}/*.* ."
