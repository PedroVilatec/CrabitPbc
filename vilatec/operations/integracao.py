...
INTEGRACOES = {
	"LEITURA_TROCA_GASOSA": {
		...
		"STATEMENT_CREATE": """CREATE TABLE IF NOT EXISTS LEITURA_TROCA_GASOSA (
					...
					duracao TIME,
					temperatura_interna NUMERIC(10,4),
	...
	"LER_CONFIG":{
		"ROTA": "/config?ativo=1&excluido=0",
		"PARAMS": {
			"codigo_cliente": "",
...
def integrar(integracao, access_token, obj):
	try:
		...
		response = requests.post(URL_BASE + rota, headers=headers, data=data)
...
def consultar(integracao, access_token, params):
	try:
		...
		rota = INTEGRACOES[integracao]["ROTA"]
		# print(URL_BASE + rota, headers)
		response = requests.get(URL_BASE + rota, headers=headers, params=params)
		if response.status_code != 200:
		...
		if result:
			for item in result:
				...
				if response["status"]:
					...
					print("Falha na Integracao: " + str(response["message"]) + str(item["_id"]), json.dumps(data, indent=4))
...
def ler(integracao, _params):
	# print("INTEGRACÃO", integracao, _params)
	data = {"username": USUARIO, "password": PASSWORD}
	headers = {'Content-Type': 'application/json'}
	...
	# print("ANTES CONSULTA", integracao, access_token, data)
	response = consultar(integracao, access_token, data)
	# print("RETORNO API integracao", response)
	if response["status"]:
