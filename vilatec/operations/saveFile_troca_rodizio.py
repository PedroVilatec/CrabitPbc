...
def falhaTroca(dados):
    ...
    try: 
        data = ""
        with open('../instance/LOG/TROCAS/falhas_troca_gasosa.txt', "r") as f:
            data = f.read()
            
        with open('../instance/LOG/TROCAS/falhas_troca_gasosa.txt', "w") as f:
            f.write("{} {}\n".format(time.strftime("%d/%m/%Y-%H:%M:%S"), dataSave)+data)

    except FileNotFoundError:
        if os.path.exists('../instance/LOG/TROCAS') == False:
            os.makedirs('../instance/LOG/TROCAS')
...
def resultadoTroca(bloco, str):
    dataSave = str
    try:
        data = ""
        with open('../instance/LOG/TROCAS/{}.txt'.format(bloco), "r") as f:
            data = f.read()
        with open('../instance/LOG/TROCAS/{}.txt'.format(bloco), "w") as f:
            f.write(dataSave+data)
