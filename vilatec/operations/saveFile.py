...
historico_telegram = os.path.join(ROOT_DIR, "../", "../", 'instance','backup_msg_telegram.json')
# print(backup_etens)
# print(last_backup_etens)
# print(historico_telegram)
def printException():
...
def grava_ramfs(dados, url):
	try:
		with open(url, 'w') as f:
			json.dump(dados, f)

	except FileNotFoundError:
		if os.path.exists(os.path.split(url)[0]) == False:
			os.makedirs(os.path.split(url)[0])
		with open(url, 'w') as f:
			json.dump(dados, f)

def load_last_state(url):
	try:
		...
		try:
			with open(last_backup_etens, 'r', encoding='utf-8') as json_file:
				return json.load(json_file)

		except FileNotFoundError:
			return {}

def load_dispositivosMqtt(url):
	try:
		with open(url, 'r', encoding='utf-8') as json_file:
			return json.load(json_file)
		...
		try:
			with open(backup_etens, 'r', encoding='utf-8') as json_file:
				return json.load(json_file)

		except FileNotFoundError:
			return {}

def historico_msg_telegram(url):
	try:
		with open(url, 'r', encoding='utf-8') as json_file:
			return json.load(json_file)
