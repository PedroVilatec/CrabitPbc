...
import datetime
INICIO_ENVIO_TELEGRAM = 7
FINAL_ENVIO_TELEGRAM = 22
def printException():
...
class Bot():
                ...
                if self.cemiterio in line:                    
                    if line.split("=",1)[1] != "":
                        ...
                        if have_internet():
                            ...
                            print("sem internet para telegram")
    
    def envia_telegram_pressao(self):   
        arquivo = os.path.abspath(os.path.join(os.path.dirname( __file__ ), '..', '..', 'instance/LOG'))
    ...
    def envia_telegram_single(self, endereco, mensagem):   
        if self.bot:
            try:
                if datetime.datetime.now().hour > INICIO_ENVIO_TELEGRAM and datetime.datetime.now().hour < FINAL_ENVIO_TELEGRAM:
